...
const seasons = [[1,2010,6],[2,2011,13],[3,2012,16],[4,2013,16],[5,2014,16],[6,2015,16],[7,2016,16],[8,2017,16],[9,2018,16],[10,2019,22],[11,2021,24]]
...
<section className="seriesHero"><div className="seriesPoster"><img src={poster} alt="Pôster de The Walking Dead" loading="eager"/><span>FILMES</span><strong>THE<br/>WALKING<br/>DEAD</strong><small>2010 — 2022</small></div><div><span className="kicker">SÉRIE • DRAMA • TERROR</span><h1>Assistir The Walking Dead Online</h1><p className="seriesMeta">2010–2022 · 11 temporadas · 177 episódios · 42 min em média</p><p className="seriesRating">★ 8,1/10 <span>Série de TV</span></p><p className="seriesDescription">Após um apocalipse zumbi, um grupo de sobreviventes liderado pelo ex-policial Rick Grimes atravessa um mundo perigoso em busca de segurança, abrigo e um novo lar.</p><div className="seriesActions"><a href="#assistir" className="seriesButton">Assistir</a><a href="#episodes" className="seriesGhost">Ver episódios</a></div></div></section>
...
<section id="episodes" className="episodeSection"><div className="seriesSectionHead"><div><span className="kicker">GUIA DE EPISÓDIOS</span><h2>Temporadas e episódios</h2></div><span className="episodeCount">11 temporadas · 177 episódios</span></div><div className="seasonGrid">{seasons.map(([number,year,count])=><section className="seasonCard" key={number}><div className="seasonTitle"><strong>Temporada {number}</strong><span>{year} · {count} episódios</span></div><div className="episodeRows">{Array.from({length:count},(_,i)=><div className="episodeRow" key={i}><span>T{String(number).padStart(2,'0')}E{String(i+1).padStart(2,'0')}</span><b>Episódio {i+1}</b><small>Guia do episódio</small></div>)}</div></section>)}</div></section>
