...
function Poster({movie,index}){return <div className="poster" style={{background:`linear-gradient(145deg,${colors[index%colors.length]},#101018)`}}><span className="posterYear">{movie.year}</span><div><small>FILMES</small><strong>{movie.title}</strong></div></div>}
...
export default function Home(){
 ...
 return <main><header><div className="nav"><Link href="/" className="brand"><span>F</span> FILMES</Link><button className="menu" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button><nav className={menu?'open':''}><Link href="#browse">Filmes</Link><Link href="#genres">Gêneros</Link><Link href="#about">Sobre</Link><Link href="/admin" className="adminLink">Admin</Link></nav></div></header>
 ...
 <section className="about" id="about"><div><span className="kicker">POR QUE FILMES</span><h2>Cinema clássico, sem complicação.</h2></div><p>O Filmes foi criado para facilitar a descoberta de obras clássicas disponíveis legalmente. Cada título pode incluir uma fonte autorizada, além de informações claras sobre sua disponibilidade e época.</p></section>
 <footer><div className="brand"><span>F</span> FILMES</div><p>Filmes clássicos para quem gosta de descobrir.</p><Link href="/admin">Admin</Link></footer>
