...
export default function Home(){
 ...
 return <main><header><div className="nav"><Link href="/" className="brand"><span>AF</span> A FILMES POBREFLIX</Link><button className="menu" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button><nav className={menu?'open':''}><Link href="#browse">Filmes</Link><Link href="/series/the-walking-dead">Séries</Link><Link href="#genres">Gêneros</Link><Link href="#about">Sobre</Link><Link href="/admin" className="adminLink">Admin</Link></nav></div></header>
 ...
 <section className="content seriesFeature" aria-labelledby="series-heading"><div className="sectionHead"><div><span className="kicker">SÉRIE EM DESTAQUE</span><h2 id="series-heading">The Walking Dead</h2></div></div><Link href="/series/the-walking-dead" className="seriesFeatureLink"><div><strong>Assistir The Walking Dead Online</strong><p>2010–2022 · 11 temporadas · 177 episódios · Drama, terror e suspense.</p></div><span aria-hidden="true">→</span></Link></section>
 ...
 <footer><div className="brand"><span>AF</span> A FILMES POBREFLIX</div><p>Filmes clássicos para quem gosta de descobrir.</p><Link href="/series/the-walking-dead">The Walking Dead</Link><Link href="/admin">Admin</Link></footer>
